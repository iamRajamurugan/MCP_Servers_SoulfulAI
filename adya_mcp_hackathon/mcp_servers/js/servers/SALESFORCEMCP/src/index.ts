...
import { ConnectionConfig,ConnectionType } from "./types/connection.js";
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    console.log("MCP Server Debug: Content of 'args' object:", JSON.stringify(args, null, 2));
    console.log("MCP Server Debug: Does 'args' contain '__credentials__'? ", '__credentials__' in args);
    console.log("MCP Server Debug: Value of args.__credentials__ directly:", (args as any).__credentials__);
    // --- END NEW LOGS ---
       // --- START MODIFICATION ---
    // Extract top-level components from the 'args' object (the full arguments payload from client).
    const {
        __credentials__, // <--- NOW WE'RE LOOKING FOR THIS KEY
        // The rest of the args will be tool-specific
        ...toolSpecificArgs // This will capture objectName, fields, whereClause etc.
    } = args; // Using 'any' for flexibility, ideally define a proper interface

    // Now, instead of selected_server_credentials?.SALESFORCE_MCP, we directly use __credentials__
    const salesforceRawCreds = __credentials__ as {
        clientId?: string;
        clientSecret?: string;
        username?: string;
        password?: string;
    ...
    // In your index.ts, inside setRequestHandler:

console.log("MCP Server: Raw request received:", JSON.stringify(request, null, 2));


console.log("MCP Server: salesforceRawCreds.username:", salesforceRawCreds.username);
console.log("MCP Server: salesforceRawCreds.password:", salesforceRawCreds.password ? '****' : 'NOT PROVIDED');
console.log("MCP Server: salesforceRawCreds.token:", salesforceRawCreds.token ? '****' : 'NOT PROVIDED');
console.log("MCP Server: salesforceRawCreds.clientId:", salesforceRawCreds.clientId);
console.log("MCP Server: salesforceRawCreds.clientSecret:", salesforceRawCreds.clientSecret ? '****' : 'NOT PROVIDED');
console.log("MCP Server: salesforceRawCreds.instanceUrl:", salesforceRawCreds.instanceUrl);

// Add this for extreme clarity
console.log("MCP Server: Type of salesforceRawCreds.username:", typeof salesforceRawCreds.username, "Length:", salesforceRawCreds.username?.length);
console.log("MCP Server: Type of salesforceRawCreds.password:", typeof salesforceRawCreds.password, "Length:", salesforceRawCreds.password?.length);
console.log("MCP Server: Type of salesforceRawCreds.token:", typeof salesforceRawCreds.token, "Length:", salesforceRawCreds.token?.length);
console.log("MCP Server: Type of salesforceRawCreds.clientId:", typeof salesforceRawCreds.clientId, "Length:", salesforceRawCreds.clientId?.length);
console.log("MCP Server: Type of salesforceRawCreds.clientSecret:", typeof salesforceRawCreds.clientSecret, "Length:", salesforceRawCreds.clientSecret?.length);

const isUsernamePasswordValid = typeof salesforceRawCreds.username === 'string' && salesforceRawCreds.username.length > 0 &&
                                typeof salesforceRawCreds.password === 'string' && salesforceRawCreds.password.length > 0;
const isClientIdSecretValid = typeof salesforceRawCreds.clientId === 'string' && salesforceRawCreds.clientId.length > 0 &&
                              typeof salesforceRawCreds.clientSecret === 'string' && salesforceRawCreds.clientSecret.length > 0;

console.log("MCP Server: isUsernamePasswordValid:", isUsernamePasswordValid);
console.log("MCP Server: isClientIdSecretValid:", isClientIdSecretValid);

if (!isUsernamePasswordValid && !isClientIdSecretValid) {
    throw new Error('Salesforce credentials not found or incomplete under "__credentials__". Either (clientId and clientSecret) OR (username and password) must be provided.');
}

// ... if it passes, it proceeds to try and connect using one of the flows

    // Extract all potential credential fields from the raw object
    const {
      clientId,
      clientSecret,
      username,
    ...
    console.log(clientId, clientSecret, username, password, token, instanceUrl);

    let salesforceConfig: ConnectionConfig;

    // Determine authentication type based on provided credentials
    if (clientId && clientSecret) {
      salesforceConfig = {
        type: ConnectionType.OAuth_2_0_Client_Credentials,
        clientId: clientId,
        clientSecret: clientSecret,
        loginUrl: instanceUrl
      };
      console.error("Detected OAuth 2.0 Client Credentials for Salesforce connection.");
    } else if (username && password) {
      salesforceConfig = {
        type: ConnectionType.User_Password,
        username: username,
        password: password,
        securityToken: token,
        loginUrl: instanceUrl
      };
      console.error("Detected Username-Password Flow for Salesforce connection.");
    } else {
      throw new Error('Insufficient Salesforce credentials. Either (clientId and clientSecret) OR (username and password) must be provided under "__credentials__".');
    }
    // Create the Salesforce connection using the dynamically determined config
    const conn = await createSalesforceConnection(salesforceConfig);
    // --- END MODIFICATION --
